...
# says the mildly flustered shopkeeper at the North Pole Toboggan
...
def read_instructions(instruction_text: list) -> list:
    hits = []
    mul_re = "mul\(\d{1,3},\d{1,3}\)"
    do_re = "do\(\)"
    dont_re = "don\'t\(\)"
    rgx = re.compile(f'{mul_re}|{do_re}|{dont_re}')
    for instruction in instruction_text:
        hits.append(rgx.findall(instruction))
    # flatten hits before returning
    return sum(hits, [])


def process_instructions(instruction_list: list) -> int:
    result = 0
...
if __name__ == '__main__':
    # filename = 'd3_example2.txt'
    filename = 'd3.txt'
    with open(filename, 'r') as f:
        instruction_text = f.readlines()
        hits = read_instructions(instruction_text)
    print(f"{hits=}")
    result = process_instructions(hits)
    print(f"{result=}")
