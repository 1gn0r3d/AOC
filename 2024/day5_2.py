...
# of each correctly-ordered update. In the above example, the correctly-
# ordered updates are:
#
# 75,47,61,53,29
# 97,61,53,29,13
...
# you add up the middle page numbers after correctly ordering just those updates?

from src.fileparser import d5_load_printing_instructions as load_instructions


def check_update(update: list[int], instructions: dict[int, list[int]]) -> bool:
    for i, page in enumerate(update):
        if i == 0:
            continue
        try:
            if any(x in update[0:i] for x in instructions[page]):
                return False
        except KeyError:
            continue
    return True
...
def middle_number(update: list[int]) -> int:
    """
    Returns the middle number of a list of integers
    """
    return update[len(update) // 2]\



def correct_order(update: list[int], instructions: dict[int, list[int]]) -> list[int]:
    """
    Reorganises the incorrect update in the correct order based on the instructions.
    """
    for i, page in enumerate(update):
        # try:
        #     print(f"{i=}, {page=}, {instructions[page]=}")
        # except KeyError:
        #     print(f"{i=}, {page=}")

        for j, previous_page in enumerate(update[0:i]):
            try:
                if previous_page in instructions[page]:
...
def process_updates(updates: list[list[int]], instructions: dict[int, list[int]]):
    number_sum = 0
    for update in updates:
        if not check_update(update, instructions):
            # print(f"{update=}")
            corrected_update = correct_order(update, instructions)
            # print(f"{corrected_update=}")
            number_sum += middle_number(corrected_update)

    return number_sum


if __name__ == '__main__':
    # filename = 'd5_example.txt'
    filename = 'd5.txt'
    instructions, updates = load_instructions(filename)
    sum = process_updates(updates, instructions)
    print(f"{sum=}")
