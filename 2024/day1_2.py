...
# locations that are historically significant to the North Pole;
...
# smallest number in the right list is another 3. The distance between them is 1.
...
def calculate_distance(lst_1: list, lst_2: list) -> int:
    total_distance = 0
    for i, j in zip(sorted(lst_1), sorted(lst_2)):
        total_distance += abs(i - j)
    return total_distance


def calculate_similarity(lst_1: list, lst_2: list) -> int:
    similarity = 0
    for entry in sorted(lst_1):
        hits = lst_2.count(entry)
        print(f"{entry=} from list 1 appears {hits} times in list 2")
        similarity += hits * entry
    return similarity


def parse_txt_file(file_path: str) -> int:
    lst_1, lst_2 = [], []
    with open(file_path, 'r') as f:
        for line in f.readlines():
            split_line = line.split(' ')
            a = int(split_line[0].strip())
            b = int(split_line[-1].strip())
            lst_1.append(a)
            lst_2.append(b)
    return lst_1, lst_2
...
def main():
    lst_1, lst_2 = parse_txt_file(file_path='d1.txt')
    # lst_1, lst_2 = parse_txt_file(file_path='d1_example.txt')
    print(calculate_distance(lst_1, lst_2))
    print(calculate_similarity(lst_1, lst_2))
